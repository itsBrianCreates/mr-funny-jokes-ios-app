...
final class LocalStorageService: @unchecked Sendable {
    ...
    // MARK: - Video Watch/Like Tracking

    private let videoWatchedKey = "videoWatched"
    private let videoLikedKey = "videoLiked"

    /// Mark a video as watched
    func markVideoWatched(firestoreId: String) {
        queue.sync {
            var watched = userDefaults.stringArray(forKey: videoWatchedKey) ?? []
            if !watched.contains(firestoreId) {
                watched.append(firestoreId)
                // Keep only last 500 watched videos
                if watched.count > 500 {
                    watched.removeFirst(watched.count - 500)
                }
                userDefaults.set(watched, forKey: videoWatchedKey)
            }
        }
    }

    /// Check if a video has been watched
    func isVideoWatched(firestoreId: String) -> Bool {
        return queue.sync {
            let watched = userDefaults.stringArray(forKey: videoWatchedKey) ?? []
            return watched.contains(firestoreId)
        }
    }

    /// Set video liked state
    func setVideoLiked(firestoreId: String, liked: Bool) {
        queue.sync {
            var likedVideos = userDefaults.stringArray(forKey: videoLikedKey) ?? []
            if liked {
                if !likedVideos.contains(firestoreId) {
                    likedVideos.append(firestoreId)
                }
            } else {
                likedVideos.removeAll { $0 == firestoreId }
            }
            userDefaults.set(likedVideos, forKey: videoLikedKey)
        }
    }

    /// Check if a video is liked
    func isVideoLiked(firestoreId: String) -> Bool {
        return queue.sync {
            let likedVideos = userDefaults.stringArray(forKey: videoLikedKey) ?? []
            return likedVideos.contains(firestoreId)
        }
    }
}
