...
must_haves:
  ...
  artifacts:
    ...
    - path: "MrFunnyJokes/MrFunnyJokes/Views/JokeFeedView.swift"
      provides: "Pull-to-refresh with full reset"
...
<task type="auto">
  <name>Task 3: Add pull-to-refresh to JokeFeedView</name>
  <files>MrFunnyJokes/MrFunnyJokes/Views/JokeFeedView.swift</files>
  <action>
Add pull-to-refresh capability to the feed:

1. Add `.refreshable` modifier to the ScrollView (before `.onChange`):
   ```swift
   .refreshable {
...
2. Keep existing LoadingMoreView and EndOfFeedView unchanged - they already handle the loading states correctly.

Note: Users can pull-to-refresh to retry if loading fails. This provides a simple, consistent recovery mechanism without additional error state tracking.
  </action>
  <verify>
Build the app:
- `xcodebuild -project MrFunnyJokes/MrFunnyJokes.xcodeproj -scheme MrFunnyJokes -destination 'platform=iOS Simulator,name=iPhone 16' build 2>&1 | tail -20`
