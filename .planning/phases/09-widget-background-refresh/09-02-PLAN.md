...
4. Add private `resolveJokeForDisplay() async -> SharedJokeOfTheDay` method:
   ...
   - Step 3: Network failed - use cached fallback via SharedStorageService.getRandomFallbackJoke()
     - If cache has jokes, return random one (THIS is the "graceful fallback" per WIDGET-03)
   - Step 4: No cache (fresh install) - return SharedJokeOfTheDay.placeholder

**WIDGET-03 clarification:** The 24-hour staleness triggers the cascade (fetch -> fallback -> placeholder).
The fallback cache (random joke from 20 cached jokes) IS the "graceful fallback for stale data" mentioned
in WIDGET-03. The placeholder is only shown as absolute last resort when fallback cache is empty
(fresh install, never opened app). This satisfies WIDGET-03 because stale data triggers fallback jokes.

5. Add private `isStale(_ date: Date) -> Bool` method:
   - Return true if Date().timeIntervalSince(date) > 86400 (24 hours)
   - Uses device clock (Date()) compared to stored lastUpdated timestamp
...
**Timezone consistency:** Uses same `TimeZone(identifier: "America/New_York")!` constant as WidgetDataFetcher
(Plan 01) for consistency. WidgetDataFetcher uses ET for "today's date" API URL, JokeOfTheDayProvider
uses ET for next refresh scheduling. Both align with the JOTD midnight ET cycle.
  </action>
  <verify>
...
Implementation exists: `grep -E "(resolveJokeForDisplay|isStale|nextMidnightET|WidgetDataFetcher)" MrFunnyJokes/JokeOfTheDayWidget/JokeOfTheDayProvider.swift`
  ...
  <done>JokeOfTheDayProvider has stale detection (24h threshold), direct fetch fallback, ET timezone refresh, and graceful degradation to cached fallback (satisfying WIDGET-03)</done>
</task>
...
<task type="auto">
  <name>Task 3: REST API Smoke Test</name>
  <files>None (verification only)</files>
  <action>
Run automated smoke test to verify Firestore REST API is accessible before human verification.

1. Execute curl command to test the REST API endpoint:
```bash
# Get today's date in ET timezone for the API path
TODAY=$(TZ="America/New_York" date +%Y-%m-%d)

# Test the daily_jokes endpoint
curl -s -o /dev/null -w "%{http_code}" \
  "https://firestore.googleapis.com/v1/projects/mr-funny-jokes/databases/(default)/documents/daily_jokes/${TODAY}"
```

2. Interpret results:
   - 200: REST API working - proceed to human checkpoint
   - 403: Security rules blocking public read - need to update Firestore rules
   - 404: Today's JOTD document not created yet - acceptable if midnight ET hasn't passed
   - Other: Network or API issue

3. If 403 error: Stop and report that Firestore security rules need updating before widgets will work.
   The user must go to Firebase Console > Firestore > Rules and ensure `daily_jokes` and `jokes` collections
   allow public read access.

4. If 200 or 404: Proceed to human checkpoint - API is accessible.

This automated check prevents wasting human time testing widgets that will fail due to API access issues.
  </action>
  <verify>
Curl returns 200 or 404: `TZ="America/New_York" curl -s -o /dev/null -w "%{http_code}" "https://firestore.googleapis.com/v1/projects/mr-funny-jokes/databases/(default)/documents/daily_jokes/$(date +%Y-%m-%d)"`
  </verify>
  <done>REST API smoke test passes (200 or 404), confirming widgets can access Firestore without 403 errors</done>
</task>
...
- REST API: Verified accessible (Task 3 smoke test)
  </what-built>
  <how-to-verify>
...
1. Watch Xcode console while widgets reload
2. No 403 errors (smoke test already verified)
3. Look for successful fetch logs if any
...
5. REST API smoke test passes (200 or 404, not 403)

6. All 6 widget sizes render correctly in simulator/device
</verification>
...
- JokeOfTheDayProvider falls back to cached joke when network fails (WIDGET-03 graceful fallback)
...
- REST API smoke test passes before human verification
