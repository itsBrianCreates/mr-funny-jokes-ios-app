...
**Timezone consistency note:** WidgetDataFetcher uses `TimeZone(identifier: "America/New_York")!` for calculating "today's date" when building the REST API URL. JokeOfTheDayProvider (Plan 02) uses the SAME timezone constant for stale detection and next refresh scheduling. This ensures consistency across both components.
...
Note: Firestore REST API allows unauthenticated reads for public collections. If 403 errors occur, security rules may need updating. REST API accessibility is tested at runtime in Plan 02's human checkpoint (Test 4) - this task only verifies compile-time correctness.
  </action>
  <verify>
...
File exists with expected structure: `grep -l "firestore.googleapis.com" MrFunnyJokes/JokeOfTheDayWidget/WidgetDataFetcher.swift`
  ...
  <done>WidgetDataFetcher.swift exists in widget target, compiles without errors, contains Firestore REST API endpoints, and uses ET timezone for date calculation</done>
</task>
...
3. WidgetDataFetcher.swift exists and uses REST API (not Firebase SDK):
   ...
   - Contains: `America/New_York` (ET timezone)
...
- WidgetDataFetcher uses ET timezone for "today's date" calculation (consistent with Plan 02)
