...
must_haves:
  ...
  key_links:
    - from: "JokeDetailSheet"
      ...
      pattern: "onSave"
    - from: "All 7 JokeDetailSheet call sites"
...
<objective>
Add Save button to JokeDetailSheet, wire onSave callback through all 7 call sites, and rewire MeView to display saved jokes instead of rated jokes.
...
<task type="auto">
  <name>Task 1: Add Save button to JokeDetailSheet and wire onSave through all 7 call sites</name>
  <files>
    MrFunnyJokes/MrFunnyJokes/Views/JokeDetailSheet.swift
    MrFunnyJokes/MrFunnyJokes/Views/JokeCardView.swift
    MrFunnyJokes/MrFunnyJokes/Views/CharacterDetailView.swift
...
**All 7 call sites** -- Add `onSave` parameter to each `JokeDetailSheet(` instantiation. Find all sites by searching for `JokeDetailSheet(` in the codebase. The research identified these locations:
...
- Update ALL JokeCardView instantiation sites (JokeFeedView, or wherever JokeCardView is used) to provide `onSave`

Search for `JokeCardView(` across the codebase to find all its instantiation sites and update them.
...
**Do not leave any compile errors.** Every instantiation site of every modified view must be updated.
  ...
  <done>Save button appears in JokeDetailSheet between rating and copy/share. All 7 JokeDetailSheet call sites pass onSave. All intermediate views (JokeCardView, RankedJokeCard, etc.) updated with onSave if they use callback patterns. Project compiles with zero errors.</done>
</task>
...
4. **Update jokeCard(for:):** Add a rating indicator to each card. After the character/category HStack, add a small rating emoji indicator if the user has rated the joke:
   ```swift
   if let rating = joke.userRating {
       HStack(spacing: 4) {
           Text(rating == 5 ? "😂" : "🤮")
           Text(rating == 5 ? "Hilarious" : "Horrible")
       }
       .font(.caption)
       .foregroundStyle(.secondary)
   }
   ```
   This fulfills the Phase 18 requirement preview -- showing rating status on saved jokes.

5. **Keep the sheet presentation:** Keep the `.sheet(isPresented:)` with JokeDetailSheet, but make sure `onSave` is wired (already done in Task 1).
...
If any other consumer exists, keep the property. Only remove if truly dead.
  ...
  <done>MeView displays saved jokes in newest-first order. Empty state says "No Saved Jokes Yet". Swipe-to-delete calls unsaveJoke (preserves rating). Rating indicator shown on each saved joke card. Dead ratedJokes/hilariousJokes/horribleJokes removed from JokeViewModel. Project compiles.</done>
</task>
...
- All 7 JokeDetailSheet call sites wire onSave to the appropriate ViewModel's saveJoke method
...
- Each saved joke card shows a rating indicator (Hilarious/Horrible) if the user rated it
